...
const BSSInputs = () => {
    ...
    const [weight, setWeight] = useState(() => {
        const localValue = localStorage.getItem("BSSWeight");
        if (localValue == null) return []
        return JSON.parse(localValue)
    });

    const [enterReps, setEnterReps] = useState("");
    const [BSSReps, setBSSReps] = useState(() => {
        const localValue = localStorage.getItem("BSSReps");
        if (localValue == null) return []
        return JSON.parse(localValue)
    });
    ...
    useEffect(() => {
        localStorage.setItem("BSSWeight", JSON.stringify(weight))
    }, [weight]);

    useEffect(() => {
        localStorage.setItem("BSSReps", JSON.stringify(BSSReps))
    }, [BSSReps]);
    ...
    function deleteWeight(id) {
        ...
        setBSSReps(currentReps => {
            return currentReps.filter(BSSReps => BSSReps.id !== id)
        })
    }
    ...
    return (
        ...
        <form onSubmit={submitFormReps}>
            <div class="input-group mb-3">
                <label htmlFor="BSSReps"></label>
                <input class="form-control" placeholder="Enter Reps" value={enterReps} onChange={e => setEnterReps(e.target.value)} type="text" id="BSSReps"/>
                <button class="btn btn-primary custom-button">Add</button>
            </div>
        </form>
